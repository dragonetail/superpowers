...
构建一个生成 ASCII 分形的 CLI 工具。完整规格请参阅 `design.md`。
...
**验证：**
- `./fractals --help` 显示用法，其中 "sierpinski" 和 "mandelbrot" 列为可用命令
- `./fractals`（无参数）显示帮助
...
- `./fractals mandelbrot --width 40 --height 12` 输出较小的版本
...
**执行：**
- 验证 Sierpinski 的 `--char` 参数将字符传递给算法
- 对于 Mandelbrot，`--char` 应使用单个字符而非渐变
- 添加自定义字符输出的测试
