...
选择你的 shell：[命令提示符](#command-prompt) | [PowerShell](#powershell) | [Git Bash](#git-bash)
...
注意：Git Bash 的原生 `ln` 命令会复制文件而不是创建符号链接。请改用 `cmd //c mklink`（`//c` 是 Git Bash 中 `/c` 的语法）。
...
如果在 WSL 内运行 OpenCode，请使用 [macOS / Linux](#macos--linux) 的说明。
...
插件通过 `experimental.chat.system.transform` 钩子自动注入 superpowers 上下文。这会在每次请求时将 "using-superpowers" 技能内容添加到系统提示中。

### 原生技能集成

Superpowers 使用 OpenCode 原生的 `skill` 工具进行技能发现和加载。技能通过符号链接到 `~/.config/opencode/skills/superpowers/`，因此它们会与你的个人和项目技能一起显示。

### 工具映射

为 Claude Code 编写的技能会自动适配到 OpenCode。引导程序提供映射说明：
...
**组件：**
- `experimental.chat.system.transform` 钩子用于引导注入
...
技能由 OpenCode 原生技能系统发现。每个技能都有一个带有 YAML frontmatter 的 `SKILL.md` 文件。
...
3. 检查技能结构：每个技能需要一个包含有效 frontmatter 的 `SKILL.md` 文件

### Windows: 模块未找到错误

如果在 Windows 上看到 `Cannot find module` 错误：
- **原因：** Git Bash 的 `ln -sf` 会复制文件而不是创建符号链接
- **解决：** 改用 `mklink /J` 目录 junction（参见 Windows 安装步骤）

### 引导程序未出现

1. 验证 using-superpowers 技能存在：`ls ~/.config/opencode/superpowers/skills/using-superpowers/SKILL.md`
...
Agent 应该提到拥有 superpowers 并能够列出 `superpowers/` 中的技能。
