...
Claude Code 插件需要在 Windows、macOS 和 Linux 上都能工作的钩子。本文档解释了实现这一目标的多语言包装器技术。
...
2. `@echo off` - 抑制命令回显
3. bash.exe 命令运行时带有：
   - `-l`（登录 shell）以获取正确的 PATH 和 Unix 工具
...
2. 直到 `CMDBLOCK` 的所有内容被 heredoc 消耗（忽略）
...
你实际的钩子逻辑放在 `.sh` 文件中。为确保它在 Windows 上工作（通过 Git Bash）：
...
对于有多个钩子的插件，你可以创建一个通用的包装器，将脚本名称作为参数：
