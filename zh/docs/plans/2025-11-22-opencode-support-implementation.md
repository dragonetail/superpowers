...
> **For Claude:** 必需的子技能：使用 superpowers:executing-plans 逐任务实现此计划。

**目标：** 使用原生 JavaScript 插件为 OpenCode.ai 添加完整的 superpowers 支持，该插件与现有 Codex 实现共享核心功能。

**架构：** 将通用技能发现/解析逻辑提取到 `lib/skills-core.js`，重构 Codex 使用它，然后使用其原生插件 API 构建 OpenCode 插件，支持自定义工具和会话钩子。
...
```bash
# 克隆 superpowers 技能到 OpenCode 配置目录
...
- `Task` with subagents → 使用 `@mention` 语法调用 OpenCode 子代理
...
- **OpenCode Support**: OpenCode.ai 的原生 JavaScript 插件
  - 自定义工具：`use_skill` 和 `find_skills`
  - 带工具映射指令的自动会话引导
...
运行：`tree -L 2 .opencode/`（如果没有 tree，使用 `find .opencode -type f`）
预期：
```
.opencode/
├── INSTALL.md
...
1. 推送到远程
2. 创建 pull request
3. 使用真实 OpenCode 安装测试（需要安装 OpenCode）

---

## 测试指南（手动 - 需要 OpenCode）

这些步骤需要安装 OpenCode，不属于自动化实现：
