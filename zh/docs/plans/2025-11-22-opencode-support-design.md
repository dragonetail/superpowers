...
OpenCode.ai 是一个类似于 Claude Code 和 Codex 的编码代理。之前尝试将 superpowers 移植到 OpenCode 的尝试（PR #93、PR #116）使用了文件复制方法。本设计采用不同的方法：使用其 JavaScript/TypeScript 插件系统构建原生 OpenCode 插件，同时与 Codex 实现共享代码。

### 平台间的关键差异

- **Claude Code**：原生 Anthropic 插件系统 + 基于文件的技能
- **Codex**：无插件系统 → 引导 markdown + CLI 脚本
- **OpenCode**：JavaScript/TypeScript 插件，支持事件钩子和自定义工具 API

### OpenCode 的代理系统

- **主代理**：Build（默认，完全访问）和 Plan（受限，只读）
- **子代理**：General（研究、搜索、多步骤任务）
- **调用方式**：由主代理自动调度 或 手动 `@mention` 语法
...
// lib/skills-core.js
module.exports = {
  extractFrontmatter(filePath),      // 从 YAML 解析名称 + 描述
  findSkillsInDir(dir, maxDepth),    // 递归 SKILL.md 发现
  findAllSkills(dirs),                // 扫描多个目录
  resolveSkillPath(skillName, dirs), // 处理覆盖（personal > core）
